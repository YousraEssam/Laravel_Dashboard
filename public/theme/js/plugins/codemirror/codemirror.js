...
(function (mod) {
  ...
  // classes.
    ...
    function updateLineGutter(cm, lineView, lineN, dims)
    {
        ...
        if (cm.options.lineNumbers || markers) {
            ...
            wrap.insertBefore(
                elt("div", null, "CodeMirror-gutter-wrapper", "position: absolute; left: " +
  ...
  // the change appears on the screen quickly.
    function fastPoll(cm)
    {
        ...
        function p()
        {
            ...
            if (!changed && !missed) {
                missed = true; cm.display.poll.set(60, p);} else {
    ...
    function onMouseDown(e)
    {
        ...
        if (eventInWidget(display, e)) {
            if (!webkit) {
                ...
                setTimeout(function () {
                    display.scroller.draggable = true;}, 100);
            }
            return;
        }
        ...
        var dragEnd = operation(cm, function (e2) {
            ...
            if (Math.abs(e.clientX - e2.clientX) + Math.abs(e.clientY - e2.clientY) < 10) {
              ...
              // Work around unexplainable focus problem in IE9 (#2127)
                if (ie && ie_version == 9) {
                    setTimeout(function () {
                        document.body.focus(); focusInput(cm);}, 20);
                }
            }
        });
  ...
  // have to be initialized before the editor can start at all.
    ...
    option("lineNumberFormatter", function (integer) {
        return integer;}, guttersChanged, true);
                ...
                for (var i = 0; i < first.length; ++i) {
                    if (first[i].to == null) {
                        (gapMarkers || (gapMarkers = [])
                            ).push(new MarkedSpan(first[i].marker, null, null));
                    }
                }
            }
            for (var i = 0; i < gap; ++i) {
  ...
  // object.
    function historyChangeFromChange(doc, change)
    {
        ...
        linkedDocs(doc, function (doc) {
            attachLocalSpans(doc, histChange, change.from.line, change.to.line + 1);}, true);
        return histChange;
    }
